...
export default function AdminIndex() {
  ...
  const handleGuardarManual = async (e: React.FormEvent) => {
    ...
    setGuardando(true);
    try {
      await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/reservas`, form, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      <div className="max-w-4xl mx-auto space-y-10">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => { setForm(FORM_INICIAL); setModalAbierto(true); }}
          ...
          <>
            ...
            <section className="space-y-4">
              <h2 className="text-xl font-bold">
                Reservas de hoy <span className="text-zinc-400 text-base font-normal">({hoy})</span>
              </h2>
              {reservasHoy.length === 0 ? (
                ...
                <div className="space-y-3">
                  {reservasHoy.map((r) => (
                    <ReservaCard key={r._id} reserva={r} onEliminar={eliminarReserva} />
                  ))}
                </div>
              )}
            </section>
      ...
      {modalAbierto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
          <div className="bg-zinc-800 rounded-2xl p-6 w-full max-w-md space-y-4 relative max-h-[90vh] overflow-y-auto">
            <button onClick={() => setModalAbierto(false)} className="absolute top-4 right-4 text-zinc-400 hover:text-white transition">
              <X size={20} />
            </button>
            ...
            <form onSubmit={handleGuardarManual} className="space-y-3">
              <input name="nombre" placeholder="Nombre" required value={form.nombre} onChange={handleFormChange}
              ...
              <input name="email" placeholder="Email" type="email" required value={form.email} onChange={handleFormChange}
                className="w-full p-3 rounded-xl bg-zinc-700 text-white placeholder-zinc-400 outline-none focus:ring-2 focus:ring-green-500" />
              <input name="telefono" placeholder="Teléfono" type="tel" required value={form.telefono} onChange={handleFormChange}
                className="w-full p-3 rounded-xl bg-zinc-700 text-white placeholder-zinc-400 outline-none focus:ring-2 focus:ring-green-500" />
              <input name="fecha" type="date" required value={form.fecha} onChange={handleFormChange}
                  ...
                  {HORARIOS.map((h) => (
                    <button key={h} type="button"
                      ...
                      className={`py-2 rounded-xl text-sm font-medium transition ${
                        form.hora === h ? "bg-green-600 text-white" : "bg-zinc-700 text-zinc-300 hover:bg-zinc-600"
                      }`}
                    >
                      {h}
                    </button>
              ...
              <input name="personas" type="number" min={1} max={20} value={form.personas} onChange={handleFormChange}
                placeholder="Personas"
                className="w-full p-3 rounded-xl bg-zinc-700 text-white outline-none focus:ring-2 focus:ring-green-500" />
              <select name="sector" value={form.sector} onChange={handleFormChange}
                className="w-full p-3 rounded-xl bg-zinc-700 text-white outline-none focus:ring-2 focus:ring-green-500">
                <option value="Patio">Patio</option>
                <option value="Esquina">Esquina</option>
              </select>
              <textarea name="comentario" placeholder="Comentario (opcional)" value={form.comentario} onChange={handleFormChange} rows={2}
  ...
  return (
    <div className="bg-zinc-800 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      <div className="space-y-1 text-sm">
        ...
        {reserva.listaEspera && (
          <span className="inline-block bg-yellow-500 text-black text-xs px-2 py-1 rounded-full font-semibold">Lista de espera</span>
        )}
      </div>
      <button onClick={() => onEliminar(reserva._id)}
